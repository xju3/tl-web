export default {
  'common.actions': '操作',
  'common.actions.edit': '编辑',
  'common.actions.view': '查看',
  'common.actions.delete': '删除',
  'common.actions.add': '新增',
  'common.actions.select': '选择',
  'common.actions.add.success': '新增成功',
  'common.actions.edit.success': '编辑成功',
  'common.actions.deleting': '正在删除',
  'common.actions.delete.success': '删除成功',
  'common.actions.saving': '正在保存',
  'common.actions.save': '保存',
  'common.actions.save.success': '保存成功',
  'common.name': '名称',
  'common.code': '编码',
  'common.ip': 'IP 地址',
  'common.port': '端口',
  'common.create': '创建',
  'common.delete.confirm': '确定要删除吗？',
  'common.description': '描述',
  'common.err.code.required': '编码为必填项',
  'common.err.name.required': '名称为必填项',
  'common.err.ip': 'ip地址格式不正确',
  'common.err.length.range': '{field}的长度必须在{min}到{max}个字符之间',
  'common.sorter.tooltip': '按住 Shift 键可以进行多字段排序',
};
